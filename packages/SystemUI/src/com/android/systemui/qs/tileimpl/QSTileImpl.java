...
public abstract class QSTileImpl<TState extends State> implements QSTile {
    ...
    public static int getColorForState(Context context, int state) {
        ...
        switch (state) {
            case Tile.STATE_UNAVAILABLE:
			    if (tintMode == 1) {
                    return Utils.getUnavailable(context,
				            getTintColor(context, android.R.attr.colorForeground, mCurrentUserId));
                } else {
            ...
            case Tile.STATE_INACTIVE:
			    if (tintMode == 1) {
                    return Utils.getDisabled(context,
					        getTintColor(context, android.R.attr.textColorHint, mCurrentUserId));
                } else {
                    return Utils.getColorAttr(context, android.R.attr.textColorHint);
                }
            case Tile.STATE_ACTIVE:
                if (tintMode == 1) {
                    return getTintColor(context, android.R.attr.textColorPrimary, mCurrentUserId));
                } else {
	...
	private int getTintColor(Context context, int fallBackColor, int mCurrentUser) {
		return Settings.Secure.getIntForUser(context.getContentResolver(), Settings.Secure.TINT_COLOR, fallBackColor, mCurrentUser);
	}
