...
public class QS_BatteryMeterDrawable extends Drawable implements
        BatteryController.BatteryStateChangeCallback {

    public static final String TAG = QS_BatteryMeterDrawable.class.getSimpleName();
    ...
    private int mIconTint = Color.DKGRAY;
    ...
    public QS_BatteryMeterDrawable(Context context, Handler handler, int frameColor) {
        // Portrait is the default drawable style
        this(context, handler, frameColor, BATTERY_STYLE_PORTRAIT);
    }

    public QS_BatteryMeterDrawable(Context context, Handler handler, int frameColor, int style) {
        mContext = context;
        mHandler = handler;
    ...
    public static int getColorAttr(Context context, int attr) {
        ...
        @ColorInt int textColor = ta.getColor(0, 0);
        ta.recycle();
        return textColor;
    }
    ...
    private void loadBatteryDrawables(Resources res, int style) {
        if (isThemeApplied()) {
            try {
                checkQS_BatteryMeterDrawableValid(res, style);
            } catch (QS_BatteryMeterDrawableException e) {
        ...
        mFrameDrawable.setTint(mCurrentBackgroundColor != 0
                ? mCurrentBackgroundColor : res.getColor(R.color.qs_tile_tint_inactive));
    ...
    private void checkQS_BatteryMeterDrawableValid(Resources res, int style) {
        ...
        try {
            ...
            throw new QS_BatteryMeterDrawableException(res.getResourceName(resId) + " is an " +
                    "invalid drawable", e);
        }

        // Check that the drawable is a LayerDrawable
        if (!(batteryDrawable instanceof LayerDrawable)) {
            throw new QS_BatteryMeterDrawableException("Expected a LayerDrawable but received a " +
        ...
        if (frame == null) {
            throw new QS_BatteryMeterDrawableException("Missing battery_frame drawble");
        }
        if (bolt == null) {
            throw new QS_BatteryMeterDrawableException(
        ...
        if (level != null) {
            ...
            if (!(level instanceof AnimatedVectorDrawable)) {
                throw new QS_BatteryMeterDrawableException("Expected a AnimatedVectorDrawable " +
            ...
            try {
                ...
                throw new QS_BatteryMeterDrawableException("Unable to perform stop motion on " +
                        "battery_fill drawable", e);
            }
        } else {
            throw new QS_BatteryMeterDrawableException("Missing battery_fill drawable");
        }
    }
    ...
    private class QS_BatteryMeterDrawableException extends RuntimeException {
        public QS_BatteryMeterDrawableException(String detailMessage) {
            super(detailMessage);
        }

        public QS_BatteryMeterDrawableException(String detailMessage, Throwable throwable) {
